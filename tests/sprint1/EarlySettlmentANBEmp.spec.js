...
const data = dataRead.sheetDate("earlysettlment")[1];
...
test('Early Settlment (ANB Customer Yes)', async ({ page }, testInfo) => {
  ...
  await servicerequest.selectTypeofSettlment("Full Settlement");
  ...
  await login.enterUsername(cred.cadminsupportteam.username);
  await login.enterPassword(cred.cadminsupportteam.password);
  await login.clickSignin();
  await home.clickServiceRequest();
  await action.searchSRNumber(srnumber);
  await action.clickSRNumberfromResult(srnumber);
  await action.assignedTo(assignToNames[12].assignTo);
